{ self, lib, ... }:
{
  perSystem =
    { pkgs, ... }:
    let
      inherit (pkgs.vulkan-validation-layers) src;

      vulkan-validation-layers-headers = pkgs.runCommand "vulkan-validation-layers-headers" { } ''
        mkdir -p $out/vulkan
        cd $out/vulkan
        cp ${src}/layers/vulkan/generated/* ./
        cp -r ${src}/layers/vulkan/{*/,vk_layer_config.*} ./
      '';
    in
    {
      packages.latencyflex-vulkan = pkgs.stdenv.mkDerivation {
  ...
  w.default.nixpkgs.overlays = [ (_: f: { inherit (self.packages.${f.sys}) latencyflex-vulkan; }) ];
}
