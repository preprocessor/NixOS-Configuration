{ lib, self, ... }:
{
  flake.modules.homeManager.desktop =
    { osConfig, ... }:
    let
      inherit (self.lib) mkMangoCfg;

      shadow_size = 10;
      scheme = osConfig.scheme;
    in
    {
      wayland.windowManager.mango.settings = mkMangoCfg {
        # Overview Setting
        hotarea_size = 10;
        enable_hotarea = 1;
        ov_tab_mode = 0;
        ...
        shadows_size = shadow_size;
        shadows_blur = shadow_size - 1;
        shadows_position_x = -shadow_size;
        shadows_position_y = -shadow_size;
